...
def normalize_nav_block(block: str, lang: str) -> str:
    ...
    if lang == "pt":
        ai_href = '/pt/ia-hai/'
        ai_anchor = '<a href="/pt/ia-hai/">IA/HCI/HAI</a>'
        block = re.sub(r'<a href="/pt/ia-hai/">[^<]*</a>', ai_anchor, block)
        publication_href = '/pt/publicacoes/'
        publication_anchor = '<a href="/pt/publicacoes/">Publicações</a>'
        archive_href = '/pt/arquivo/'
        archive_anchor = '<a href="/pt/arquivo/">Arquivo</a>'
    else:
        ai_href = '/en/ai-hai/'
        ai_anchor = '<a href="/en/ai-hai/">AI/HCI/HAI</a>'
        block = re.sub(r'<a href="/en/ai-hai/">[^<]*</a>', ai_anchor, block)
        publication_href = '/en/publications/'
        publication_anchor = '<a href="/en/publications/">Publications</a>'
        archive_href = '/en/archive/'
        archive_anchor = '<a href="/en/archive/">Archive</a>'

    if f'href="{ai_href}"' not in block:
        if f'href="{publication_href}"' in block:
            block = block.replace(publication_anchor, ai_anchor + publication_anchor, 1)
        else:
            block = block.replace('</nav>', ai_anchor + '</nav>', 1)

    if f'href="{publication_href}"' not in block:
        if f'href="{archive_href}"' in block:
            block = block.replace(archive_anchor, publication_anchor + archive_anchor, 1)
        else:
            block = block.replace('</nav>', publication_anchor + '</nav>', 1)

    if f'href="{archive_href}"' not in block:
        if f'href="{publication_href}"' in block:
            block = block.replace(publication_anchor, publication_anchor + archive_anchor, 1)
        else:
            block = block.replace('</nav>', archive_anchor + '</nav>', 1)
    return block
